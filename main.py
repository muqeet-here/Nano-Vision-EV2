...
def detect_edges(frame):
    ...
    show_image("hsv", hsv)
    lower_blue = np.array([0, 100 * i, 100])
...
def detect_line_segments(cropped_edges):
    ...
    line_segments = cv2.HoughLinesP(cropped_edges, rho, angle, min_threshold, np.array([]), minLineLength=11,
                                    maxLineGap=4)
...
if __name__ == '__main__':
    ...
    try:
        while cap.isOpened():
            ...
            #         if label == 'Pothole':
            #             cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 0, 255), 2)
            #             cv2.putText(frame, label + ' ' + str(conf), (x1, y1 - 5), cv2.FONT_HERSHEY_PLAIN, 2,
            #                         (0, 0, 255), 2)
